...
def _get_func_bytes_mtx_to_processed_bytes_and_other_settings_based_on_file_format( file_format, dtype_of_row_and_col_indices = None, dtype_of_value = None ) :
    ...
    if file_format == 'mtx_gzipped' :
        ...
        def func_bytes_mtx_to_processed_bytes( bytes_mtx ) :
            return gzip_bytes( _pickle_obj_to_bytes( _bytes_mtx_to_arrays_mtx( bytes_mtx, dtype_of_row_and_col_indices, dtype_of_value ) ) )
...
def _get_func_arrays_mtx_to_processed_bytes_and_other_settings_based_on_file_format( file_format, str_format_value = '{}', dtype_of_row_and_col_indices = None, dtype_of_value = None ) :
    ...
    if file_format == 'mtx_gzipped' :
        ...
        def func_arrays_mtx_to_processed_bytes( arrays_mtx ) :
            return gzip_bytes( _pickle_obj_to_bytes( _arrays_mtx_to_arrays_mtx( arrays_mtx, dtype_of_row_and_col_indices, dtype_of_value ) ) )
...
def _get_func_processed_bytes_to_arrays_mtx_and_other_settings_based_on_file_format( file_format, dtype_of_row_and_col_indices = None, dtype_of_value = None ) :
    ...
    if file_format == 'mtx_gzipped' :
        def func_processed_bytes_to_arrays_mtx( bytes_content ) :
            return _bytes_mtx_to_arrays_mtx( gunzip_bytes( bytes_content ), dtype_of_row_and_col_indices, dtype_of_value )
    elif file_format == 'feather_lz4' :
        def func_processed_bytes_to_arrays_mtx( bytes_content ) :
...
def sort_mtx( path_file_gzip, path_file_gzip_sorted = None, int_num_records_in_a_chunk = 10000000, int_buffer_size = 300, compresslevel = 6, flag_dtype_is_float = False, flag_mtx_sorted_by_id_feature = True, int_num_threads_for_chunking = 5, int_num_threads_for_writing = 1, int_max_num_input_files_for_each_merge_sort_worker = 8, int_num_chunks_to_combine_before_concurrent_merge_sorting = 8, za_mtx = None, za_mtx_index = None ) :
    ...
    # delete temp folder
    filesystem_operations( 'rm', path_folder_temp )
def create_zarr_from_mtx( path_file_input_mtx, path_folder_zarr, int_buffer_size = 1000, int_num_workers_for_writing_ramtx = 10, chunks_dense = ( 1000, 1000 ), dtype_mtx = np.float64, flag_combine_duplicate_records : bool = False ) :
    """ # 2023-01-27 12:30:25 
    ...
    def __distribute( pipe_receiver, int_num_workers_for_writing_ramtx, int_num_barcodes_in_a_chunk ) :
        ...
        def __write_zarr( pipe_receiver ) :
            ...
            while True :
                ...
                if flag_combine_duplicate_records : # handle duplicate records (combine values of duplicate records)
                    # combine values for each unique set of coordinates
                    dict_t_coords_to_combined_value = dict( )
                    for coord_bc, coord_ft, value in zip( coords_barcodes, coords_features, values ) :
...
def zarr_object_server( path_folder_zarr : Union[ None, str ], pipe_receiver_input, pipe_sender_output, mode : str = 'r', shape = None, chunks = None, dtype = np.int32, fill_value = 0, path_process_synchronizer : Union[ str, None ] = None ) :
    """ # 2023-04-19 01:33:09 
    ...
    if path_folder_zarr is not None : # if a valid path to the Zarr object has been given 
        ...
        if hasattr( za, 'shape' ) :
            pipe_sender_output.send( ( za.shape, za.chunks, za.dtype, za.fill_value ) ) # send basic information about the zarr array
            flag_zarr_array = True # set the flag to True
        else :
            pipe_sender_output.send( ( None, None, None, None ) ) # return None values for a zarr group
    ...
    while True :
        ...
        if name_func == 'open' : # open a new zarr object
            path_folder_zarr, mode, shape, chunks, dtype, fill_value, path_process_synchronizer = args # parse arguments
            # %% OPEN ZARR %% # open a zarr object
            ps_za = None if path_process_synchronizer is None else zarr.ProcessSynchronizer( path_process_synchronizer ) # initialize process synchronizer (likely based on a local file system)
            if mode != 'r' : # create a new zarr object
                if shape is None or chunks is None : # if one of the arguments for opening zarr array is invalid, open zarr group instead
            ...
            if name_func == 'get_attrs' :
                ...
                pipe_sender_output.send( dict( ( key, za.attrs[ key ] ) for key in args if key in set_keys ) ) # return a subset of metadata using the list of key values given as 'args'
            elif name_func == 'set_attrs' :
                # update the metadata
                for key in kwargs :
                    za.attrs[ key ] = kwargs[ key ]
                pipe_sender_output.send( None ) # indicate the operation has been completed
...
class ZarrServer( ) :
    ...
    def __init__( self, path_folder_zarr, mode = 'r', shape = None, chunks = None, dtype = np.int32, fill_value = 0, flag_spawn : bool = True, path_process_synchronizer : Union[ str, None ] = None ) :
        ...
        if self.flag_spawn :
            ...
            if path_folder_zarr is None : # if 'path_folder_zarr' is invalid 
                self.shape, self.chunks, self.dtype, self.fill_value, self._za = None, None, None, None, None # set attributes to None values to indicate the Zarr object has not been opened
    ...
    def open( self, path_folder_zarr, mode = 'r', shape = None, chunks = None, dtype = np.int32, fill_value = 0, path_process_synchronizer : Union[ str, None ] = None, reload : bool = False ) :
        """ # 2023-04-20 02:08:57 
        ...
        if not reload and path_folder_zarr == self.path_folder and self._mode == mode :
            return
        # open Zarr object
        args = path_folder_zarr, mode, shape, chunks, dtype, fill_value, path_process_synchronizer # compose arguments
        if self.flag_spawn :
            # %% PROCESS SPAWNING %%
            ...
            if hasattr( za, 'shape' ) : # if zarr object is an array
                ...
                self.shape, self.chunks, self.dtype, self.fill_value = None, None, None, None
    ...
    def get_attrs( self, * keys ) : 
        ...
        if self.flag_spawn :
            ...
            return dict( ( key, self._za.attrs[ key ] ) for key in keys if key in set_keys ) # return a subset of metadata using the list of key values given as 'args'
    def get_attr( self, key ) : 
        ...
        if key not in dict_attrs :
            raise KeyError( f'attribute {key} does not exist in the zarr object.' ) # raise a key error if the key does not exist        

...
def create_ramtx_from_mtx( 
    ...
    path_file_input_bc, path_file_input_feature, path_file_input_mtx = MTX_Get_path_essential_files( path_folder_mtx_10x_input )
    ...
    if mode.lower( ) == 'dense' : # build dense ramtx based on the setting.
        create_zarr_from_mtx( path_file_input_mtx, f'{path_folder_output}matrix.zarr', int_buffer_size = int_buffer_size, chunks_dense = chunks_dense, dtype_mtx = dtype_dense_mtx, int_num_workers_for_writing_ramtx = int_num_threads_for_chunking, flag_combine_duplicate_records = flag_combine_duplicate_records )
        ...
        for index_chunk, df in enumerate( pd.read_csv( f"{path_folder_mtx_10x_input}{name_axis}.tsv.gz", sep = '\t', header = None, chunksize = int_num_of_entries_in_a_chunk_metadata ) ) : # read chunk by chunk        
            ...
            for index_col, arr_val in enumerate( values_str.T ) :
                with open( f"{path_folder_str_chunks}{index_chunk}.{index_col}", 'wt' ) as newfile : # similar organization to zarr
                    newfile.write( base64_encode( gzip_bytes( ( '\n'.join( arr_val ) + '\n' ).encode( ) ) ) )
            # set num_and_cat.zdf
            if values.shape[ 1 ] > 2 :
...
class ZarrDataFrame( ) :
    ...
    def __init__( 
        ...
        if zdf_template is not None :
            ...
            self._flag_does_not_wait_and_raise_error_when_modification_is_not_possible_due_to_lock = zdf_template._flag_does_not_wait_and_raise_error_when_modification_is_not_possible_due_to_lock
            self._float_second_to_wait_before_checking_availability_of_a_spin_lock = zdf_template._float_second_to_wait_before_checking_availability_of_a_spin_lock
            self.flag_store_64bit_integer_as_float = zdf_template.flag_store_64bit_integer_as_float
            ...
            self.flag_retrieve_categorical_data_as_integers = zdf_template.flag_retrieve_categorical_data_as_integers
            ...
            self._flag_load_data_after_adding_new_column = zdf_template._flag_load_data_after_adding_new_column
            self._ba_filter = None # initialize the '_ba_filter' attribute
            self.verbose = zdf_template.verbose
            self.int_max_num_entries_per_batch = zdf_template.int_max_num_entries_per_batch
            self._zdf_source = zdf_template._zdf_source
            self._flag_use_lazy_loading = zdf_template._flag_use_lazy_loading
            self._flag_spawn = zdf_template._flag_spawn if flag_spawn is None else flag_spawn # prioritize argument over the settings of the template
            ...
            self.index_zdf_data_source_when_interleaved = zdf_template.index_zdf_data_source_when_interleaved
            self._l_dict_index_mapping_interleaved = zdf_template._l_dict_index_mapping_interleaved
            self._l_dict_index_mapping_from_combined_to_component = zdf_template._l_dict_index_mapping_from_combined_to_component
            self._l_dict_index_mapping_from_component_to_combined = zdf_template._l_dict_index_mapping_from_component_to_combined

            if flag_mask and self._path_folder_mask is not None : # set the current ZarrDataFrame as a mask ZarrDataFrame
                # %% MASK %%
                self._path_folder_zdf = self._path_folder_mask # use path to the mask as the zdf path
                self._mode = 'a', 
                self._path_folder_mask = None, 
                self._flag_is_read_only = False,
                self._zdf_source = zdf_template, # use the reference to the ZarrDataFrame used as a template  to current source zdf
        else :
            # set attributes that can be changed anytime during the lifetime of the object
        ...
        self.fs = FileSystemServer( flag_spawn = self.flag_spawn, dict_kwargs_credentials_s3 = self._dict_kwargs_credentials_s3 ) # initialize the file system server
    ...
    def get_fork_safe_version( self, flag_use_locking : bool = True ) :
        ...
        return ZarrDataFrame( zdf_template = self, flag_spawn = self.is_remote, zarrspinlockserver = True if flag_use_locking else None ) # use spawning only for remote datasets
    ...
    def metadata_columns( self ) :
        ...
        if self.is_combined :
            if self.is_interleaved :
                ...
                metadata_columns = zdf.metadata_columns | metadata_columns # add metadata of the columns of the data source zdf
            else :
                # %% COMBINED STACKED %%
                for zdf in self._l_zdf : # for each zdf component
                    metadata_columns = zdf.metadata_columns | metadata_columns # add metadata of the columns of the zdf component
    ...
    def get_metadata( self ) :
        ...
        if self.use_locking : # when locking has been enabled, read metadata from the storage, and update the metadata currently loaded in the memory
            ...
            self._dict_metadata = self._zs.get_attr( 'dict_metadata' ) # retrieve 'dict_metadata' from the storage
    ...
    def get_column_metadata( self, name_col : str ) :
        ...
        if name_col in self.columns_excluding_components : # if the current column is present in the current object
            ...
            # read metadata
            self._zs.open( f"{self._path_folder_zdf}{name_col}/", mode = 'r' ) # read data from the Zarr object
            dict_col_metadata = self._zs.get_attr( 'dict_col_metadata' ) # retrieve metadata of the current column
    ...
    def set_column_metadata( self, name_col : str, dict_col_metadata : dict ) :
        ...
        if name_col in self.columns_excluding_components : # if the column is located in the current object
            ...
            try :
                ...
                self._zs.open( f"{self._path_folder_zdf}{name_col}/", mode = 'a' ) # open the Zarr object
                self._zs.set_attrs( dict_col_metadata = dict_col_metadata ) # set metadata of the current column
    ...
    def update_column_metadata( self, name_col : str, dict_col_metadata_to_be_updated : dict, flag_relpace_dict_metadata_description : bool = False ) :
        ...
        if name_col in self.columns_excluding_components : # if the column is present in the current object
            ...
            try :
                # read metadata
                self._zs.open( f"{self._path_folder_zdf}{name_col}/", mode = 'a' ) # read data from the Zarr object
                dict_col_metadata = self._zs.get_attr( 'dict_col_metadata' ) # get attributes
                ...
                self._zs.set_attrs( dict_col_metadata = dict_col_metadata ) # save the column metadata
            finally:
                # %% FILE LOCKING %%
                if self.use_locking : # if locking is used, release the lock
                    if flag_lock_acquired:
    ...
    def lazy_load( self, queries, flag_mode_write : bool = False, name_col_sink : Union[ str, None ] = None, path_column_sink : Union[ str, None ] = None, path_column_source : Union[ str, None ] = None, l_path_column_source : Union[ list, None ] = None, name_col_availability : Union[ None, str ] = None, flag_retrieve_from_all_interleaved_components : bool = False ) -> None :
        ...
        if zarr_exists( path_column_sink ) :
            ...
            dict_attrs = za_sink.get_attrs( 'flag_is_being_lazy_loaded' ) # retrieve attributes
            if 'flag_is_being_lazy_loaded' not in dict_attrs : # if the column has not been marked with a flag indicating lazy-loading has been started, exit
                return
            elif not dict_attrs[ 'flag_is_being_lazy_loaded' ] : # if lazy-loading has been completed, exit
                return
        ...
        if flag_mode_write : # for write operation
            ...
            if len( l_int_entry_that_needs_fetching ) > 0 :
                ...
                if self.is_mask and self.is_combined :
                    ...
                    za_source.open( path_column_source, mode = 'r' ) # open source zarr object
                    # open sink zarr object
                    if not zarr_exists( path_column_sink ) :
                        za_sink.open( path_column_sink, mode = 'a', shape = za_source.shape, chunks = za_source.chunks, fill_value = za_source.fill_value, dtype = str if za_source.dtype is np.dtype( 'O' ) else za_source.dtype )
                        za_sink.set_attrs( flag_is_being_lazy_loaded = True ) # update metadata of the sink column
                    else :
                    ...
                    for int_index_component, zdf, dict_index_mapping_from_combined_to_component, path_column_source in zip( np.arange( len( self._l_zdf ) ), self._l_zdf, self._l_dict_index_mapping_from_combined_to_component, l_path_column_source ) :
                        ...
                        if flag_mode_internal : # internal mode
                            ...
                            if not zarr_exists( path_column_sink ) :
                                ...
                                za_sink.open( path_column_sink, mode = 'a', shape = tuple( [ self._n_rows_unfiltered ] + list( za_source.shape[ 1 : ] ) ), chunks = tuple( [ self.get_int_num_rows_in_a_chunk( dtype, chunks_not_primary_axis = chunks_not_primary_axis ) ] + chunks_not_primary_axis ), fill_value = za_source.fill_value, dtype = dtype )
                                za_sink.set_attrs( flag_is_being_lazy_loaded = True ) # update metadata of the sink column
                            else :
...
#             logger.info( f'[ZarrDataFrame] a column for quering integer indices was written' )
        ...
        if name_col not in self.columns_excluding_components : # if the column does not exists in the current ZarrDataFrame (excluding component zdf objects )
            try :
                ...
                if not flag_dry_run : # initialize the given column # if 'flag_dry_run' is True, does not initialize the column. instead, retrieve data for initialization 
                    za.open( path_folder_col, mode = 'a', shape = shape, chunks = chunks, dtype = dtype, fill_value = fill_value ) # create a new Zarr object if the object does not exist.
    ...
    def __getitem__( self, args ) :
        ...
        if name_col in self : # if name_col is valid
            if name_col in self._loaded_data and not flag_indexing_primary_axis : # if a loaded data (filtered/unfiltered, according to the self.filter) is available and indexing is not active, return the cached data
                ...
                za.open( f"{self._path_folder_zdf}{name_col}/", mode = 'r' ) 
    ...
    def __setitem__( self, args, values ) :
        ...
        if self._flag_use_lazy_loading : # use lazy-loading when only partial data will be retrieved
            ...
            if self._mask is not None : # if mask is available, save new data to the mask
                if name_col in self and name_col not in self._mask : # if the 'name_col' exists in the current ZarrDataFrame and not in mask, copy the column to the mask
                    zarr_copy( f"{self._path_folder_zdf}{name_col}/", f"{self._mask._path_folder_zdf}{name_col}/" ) # copy zarr object from the source to the mask # ❤️ modify here to be compatible with remote zarr objects/multiprocessing
                    self._mask._add_column( name_col ) # manually add column label to the mask
                self._mask[ args ] = values # set values to the mask
                return # exit
        ...
        try :
            ...
            if flag_col_already_exists :
                ...
                dict_col_metadata = za.get_attr( 'dict_col_metadata' ) # load previous written metadata
                ...
                if not isinstance( values, np.ndarray ) or str( dtype ) == 'object' : 
                    if isinstance( values, ( str, float, int ) ) : # if a single value has been given
                        ...
                        if isinstance( val, np.ndarray ) : # check if a list of numpy arrays were given
                            dtype = val.dtype # use the dtype of the first entry
                        elif dtype is float and str( val ) == 'nan' : # detect nan values # check whether the array contains strings with np.nan values (make sure array starting with np.nan is not a string array containing np.nan values)
                            for t_coord, val in np.ndenumerate( values ) : # np.ndenumerate can handle nexted lists
                                if type( val ) is str :
                                    dtype = str
                                    break
                        elif np.issubdtype( dtype, np.integer ) : # if the first value is integer dtype, check all the values to ensure all the values are of integer dtype
                            for t_coord, val in np.ndenumerate( values ) : # np.ndenumerate can handle nexted lists
            ...
            if not flag_broadcasting_active and hasattr( values, '__getitem__' ) and hasattr( values[ 0 ], '__iter__' ) and values.ravel( ).shape[ 0 ] == values.shape[ 0 ] :
                values = np.array( list( values ), dtype = object if dtype is str else dtype ) # use 'object' dtype when converting values to a numpy.ndarray object if dtype is 'str' # convert back to list and conver to numpy array to remove the nested structure
            ...
            if dtype is str and self._dict_metadata[ 'flag_store_string_as_categorical' ] : # storing categorical data   
                ...
                za.open( path_folder_col, mode = 'a' ) if flag_col_already_exists else za.open( path_folder_col, mode = 'w', shape = shape_inferred, chunks = chunks_inferred, dtype = dtype, fill_value = fill_value ) # create a new Zarr object if the object does not exist.
                ...
                if dtype != za.dtype : # dtype should be larger than za.dtype if they are not equal (due to increased number of bits required to encode categorical data)
                    ...
                    za_new.open( path_folder_col_new, mode = 'w', shape = za.shape, chunks = __replace_chunk_size_for_the_primary_axis( za.chunks, int_num_rows_in_a_chunk ), dtype = dtype ) # create a new Zarr object using the new dtype # use the new 'int_num_rows_in_a_chunk' from the new dtype
                    za_new[ : ] = za[ : ] # copy the data 
                    self.fs.filesystem_operations( 'rm', path_folder_col ) # delete the previous Zarr object
                    self.fs.filesystem_operations( 'mv', path_folder_col_new, path_folder_col ) # replace the previous Zarr object with the new object
                    za.open( path_folder_col, mode = 'a', reload = True ) # re-open the new Zarr object
    ...
    def __delitem__( self, name_col : str ) :
        ...
        if name_col in self : # if the given name_col is valid
            ...
            path_prefix_col = f'{self._path_folder_zdf}{name_col}' # define prefix for column
            if self.use_locking : # %% FILE LOCKING %%
                self._zsls.wait_lock( f'{path_prefix_col}.lock' ) # wait until the lock becomes available (the column is now ready for 'delete' operation)
            if self.fs.filesystem_operations( 'exists', path_prefix_col + '/' ) : # delete the folder if the folder exists
    ...
    def df( self ) :
        ...
        return df
    def update( self, df, flag_use_index_as_integer_indices : bool = True, dict_name_col_to_metadata_description : Union[ None, dict ] = None ) :
        """ # 2023-03-31 17:14:56 
        ...
        for name_col in l_name_col :
            self._add_column( name_col, dict_name_col_to_metadata_description[ name_col ] if isinstance( dict_name_col_to_metadata_description, dict ) and name_col in dict_name_col_to_metadata_description else None )
    ...
    def copy_column( self, name_col_src : str, name_col_dst : str, zdf_dst = None, int_num_rows_in_a_chunk : Union[ None, int ] = 10000 ) :
        """ # 2023-02-16 20:24:58 
        ...
        if int_num_rows_in_a_chunk is None : # copy the column in a single batch
            ...
            ba_src = self.all( flag_return_valid_entries_in_the_currently_active_layer = False ) if self.filter is None else self.filter
            ...
            def __flush( ) : # flush the batch
                zdf_dst[ name_col_dst, ns[ 'l_idx_dst' ] ] = self[ name_col_src, ns[ 'l_idx_src' ] ] # fetch and copy a chunk of the data for the current batch
    ...
    def save( self, path_folder_zdf : str, l_name_col : Union[ None, list ] = None ) :
        ...
        def __work( pipe_receiver, pipe_sender ) :
            """ # 2023-01-20 13:10:40 
            """
            zdf_dst_fork_safe = zdf_dst.get_fork_safe_version( ) # get the fork-safe version
            while True :
                ins = pipe_receiver.recv( )
                if ins is None :
                    break
                name_col = ins # parse 'ins'
                self.copy_column( name_col_src = name_col, name_col_dst = name_col, zdf_dst = zdf_dst_fork_safe ) # copy column by column to the output ZarrDataFrame object
                pipe_sender.send( True )
    ...
    def resize( self, int_num_rows_new : int, flag_avoid_shrinking : bool = True ) :
        ...
        def _resize_column( name_col ) :
            ...
            """
            za.open( f"{self._path_folder_zdf}{name_col}", mode = 'a' )
    ...
    def get_categorical_data_as_integers( self, name_col : str ) :
        ...
        if len( l_cat ) == 0 :
            if self.verbose :
                logger.error( f"categories are not available for the current column '{name_col}', exiting" )
            raise KeyError( f"categories are not available for the current column '{name_col}', exiting" )
        ...
        self.flag_retrieve_categorical_data_as_integers = flag_retrieve_categorical_data_as_integers_back_up # restore the previous settings
        return arr_int_cat
    def map_category_to_entries( self, name_col : str, flag_use_integer_representation_of_category : bool = False, flag_return_dict_cat_to_num_entries : bool = False ) :
        """ # 2023-03-12 01:29:33 
    ...
    def lock( self, * l_name_col ) :
        ...
        if not self.use_locking : # if lock is not used, exit
            return
        # %% FILE LOCKING %%
        set_name_col_newly_locked = set( ) # collect the name of the columns that were newly locked.
        for name_col in l_name_col : # for each column
            if name_col not in self.columns_excluding_components : # does not lock the column that are present in the current object.
                continue
            # acquire lock of the column of the lock has not been acquired.
            path_lock = f'{self.path_folder}{name_col}.lock/'
            if path_lock not in self.locking_server.currently_held_locks : # acquire lock if it has not been acquired before the operation
                self.locking_server.acquire_lock( path_lock )
                set_name_col_newly_locked.add( name_col ) # add the column to the set of name_col with newly acquired locks
    ...
    def unlock( self, * l_name_col ) :
        ...
        if not self.use_locking : # if lock is not used, exit
            return
        # %% FILE LOCKING %%
        set_name_col_released = set( ) # collect the name of the columns that were newly locked.
        for name_col in l_name_col : # for each column
            if name_col not in self.columns_excluding_components : # does not lock the column that are present in the current object.
                continue
            # release the lock of the column of the lock has been acquired.
            path_lock = f'{self.path_folder}{name_col}.lock/'
            if path_lock in self.locking_server.currently_held_locks : # release the lock if the lock has been acquired by the current object
                self.locking_server.release_lock( path_lock )
                set_name_col_released.add( name_col ) # add the column to the set of name_col with newly acquired locks
...
class RamDataAxis( ) :
    ...
    def __init__( 
        ...
        flag_is_read_only : bool = False, 
        dict_kw_zdf : dict = { 'flag_retrieve_categorical_data_as_integers' : False, 'flag_load_data_after_adding_new_column' : True, 'flag_enforce_name_col_with_only_valid_characters' : False, 'int_max_num_entries_per_batch' : 1000000 }, 
    ...
    def get_filter( self, name_col_filter : str ) :
        ...
        'name_col_filter' : name of the column of the metadata ZarrDataFrame containing the filter
        """
        return self._convert_to_bitarray( self.meta[ name_col_filter, : ] if name_col_filter in self.meta else self.none( ) ) # if a given column name does not exist in the current metadata ZarrDataFrame, return an empty filter
    ...
    def subsample( self, float_prop_subsampling : float = 1, ba_to_subsample : Union[ None, bitarray ] = None ) :
        """ # 2023-03-11 21:42:54 
    ...
    def filters( self, flag_excluding_components : bool = False ) :
        ...
        """
        metadata_columns = self.meta.metadata_columns_excluding_components if flag_excluding_components else self.meta.metadata_columns # retrieve 'metadata_columns'
        
        l_name_col = list( ) # initialize the list
        for name_col in metadata_columns :
            dict_meta = metadata_columns[ name_col ] # retrieve metadata of the current column
...
class RamData( ) :
    ...
    def __init__( 
        ...
        dict_kw_zdf : dict = { 'flag_retrieve_categorical_data_as_integers' : False, 'flag_load_data_after_adding_new_column' : False, 'flag_enforce_name_col_with_only_valid_characters' : False }, 
        dict_kw_view : dict = { 'float_min_proportion_of_active_entries_in_an_axis_for_using_array' : 0.1, 'dtype' : np.int32 }, 
        ...
        self.bc = RamDataAxis( 
            ...
            l_ax = list( ram.bc for ram in self._l_ramdata ) if self._l_ramdata is not None else None, 
            ** dict_setting
        )
        self.ft = RamDataAxis( 
            path_folder_ramdata, 
            ...
            l_ax = list( ram.ft for ram in self._l_ramdata ) if self._l_ramdata is not None else None, 
            ** dict_setting
        )
...
#             self.set_metadata( self._dict_metadata )
    ...
    def initialize_workers( self ) :
        ...
        initialize workers
        """
        if not hasattr( self, 'flag_workers_initialized' ) : # initialize workers only once
            context = None # initialize the context
            if self.is_remote :
                context = mp.get_context( 'spawn' )

            self.executor = concurrent.futures.ProcessPoolExecutor( mp_context = context ) # retrieve executor
            self.flag_workers_initialized = True # set the flag indicating workers have been implemented.
    ...
    def save_model( self, model, name_model : str, type_model : Literal[ 'ipca', 'pumap', 'hdbscan', 'knn_classifier', 'knn_embedder', 'knngraph', 'knnindex', 'deep_learning.keras.classifier', 'deep_learning.keras.embedder' ], dict_metadata_description : dict = dict( ) ) :
        ...
        dict_metadata_description[ 'identifier_of_the_ramdata_of_origin' ] = self.identifier # record the ramdata of origin
                ...
                if name_layer not in self.layers_excluding_components : 
                    ...
                    for int_index_ram, ram in enumerate( self._l_ramdata ) :
                        if name_layer in ram.layers :
                            if self.verbose :
                                logger.info( f"loading a layer '{name_layer}' of RamData component {int_index_ram}" )
    ...
    def summarize( 
        ...
        ** warning ** existing columns will be overwritten!
        ...
        if name_layer not in self.layers :
            if self.verbose :
                logger.error( f"invalid argument 'name_layer' : '{name_layer}' does not exist." )
            return -1 
        if axis not in { 0, 'barcode', 1, 'feature', 'barcodes', 'features', 'bc', 'ft', 'b', 'f' } :
            if self.verbose :
                logger.error( f"invalid argument 'axis' : '{axis}' is invalid." )
        ...
        try :
            ...
            zdf_meta_without_locking = ax.meta.get_fork_safe_version( flag_use_locking = False ) # retrieve ZarrDataFrame metadata that does not using locking
            def post_process_batch( res ) :
                ...
                # update metadata
                df = pd.DataFrame( dict_data, index = l_int_entry_of_axis_for_querying ) # compose dataframe using 'dict_data'
                df.rename( columns = dict( ( name_col, name_col_with_prefix_and_suffix ) for name_col, name_col_with_prefix_and_suffix in zip( l_name_col_summarized, l_name_col_summarized_with_name_layer_prefix_and_suffix ) ), inplace = True ) # rename column names
                zdf_meta_without_locking.update( df, flag_use_index_as_integer_indices = True )
                del df
    ...
    def subset( 
        ...
        ** kwargs
        ...
        if not isinstance( set_id_model, set ) :
            set_id_model = set( dict_metadata_models_excluding_components ) # retrieve a set of valid 'id_model'
        set_id_model = set( e for e in set_id_model if e.rsplit( '|', 1 )[ 1 ] in set_type_model and e in dict_metadata_models_excluding_components and dict_metadata_models_excluding_components[ e ][ 'identifier_of_the_ramdata_of_origin' ] == self.identifier ) # exclude the id_model with invalid type_model, included only id_model in 'models_excluding_components', and belonging to the current ramdata
        ...
        for id_model in set_id_model : # for each valid model
            ...
            if 'filter' in model : # load filter for building the model.
                ...
                if ax.filter is None : # if all entries are active in the source RamData, all entries in the destination RamData will be used.
                    ba_subset = model[ 'filter' ] # filter of the destination RamData
                else :
                    ba_subset = ax_subset.none( )
                    for index_subset, index in enumerate( BA.find( ax.filter ) ) : # retrieve indices of input and output ramdata objects
            ...
            if type_model == 'knnindex' :
                ...
                if not ( model[ 'name_col_x' ] in ax_subset.columns and ( model[ 'name_col_filter_for_collecting_neighbors' ] is None or model[ 'name_col_filter_for_collecting_neighbors' ] in ax_subset.columns ) ) :
                    if self.verbose :
                ...
                ram_subset.train_knn(
                    ...
                    name_col_filter_for_collecting_neighbors = model[ 'name_col_filter_for_collecting_neighbors' ],
                    int_num_nearest_neighbors_to_collect = model[ 'int_num_nearest_neighbors_to_collect' ],
                )
    ...
    def identify_highly_variable_features( 
        ...
        if name_col_batch is not None :
            if name_col_batch not in self.bc.columns : # if 'name_col_batch' does not exist
                name_col_batch = None
        ...
        if name_col_filter is not None and name_col_filter in ax.meta : # if the output already exists, load the filter (if relevant setting is on) and exit
            if flag_load_filter : # load filter according to the filter
                ax.change_filter( name_col_filter )
            return

        if name_col_batch is not None and name_col_batch in ax_not_for_querying.meta : # if 'name_col_batch' is a valid column
            """
            ...
            dict_batch_to_count = ax_not_for_querying.meta.count_category( name_col_batch, flag_use_integer_representation_of_category = True ) # use integer representations
            l_name_batch = ax_not_for_querying.meta.get_categories( name_col_batch ) # retrieve the names of the batches
            int_num_batches = len( l_name_batch )

            # retrieve batch information of barcodes
            ax_not_for_querying.meta.load_as_dict( name_col_batch, flag_retrieve_categorical_data_as_integers = True ) # use integer representations
            dict_batch = ax_not_for_querying.meta.dict.pop( name_col_batch ) 
            l_batch_active = list( set( dict_batch[ k ] for k in dict_batch ) ) if isinstance( dict_batch, dict ) else np.unique( dict_batch ) # retrieve the list of active batches

            # initialize output columns
            name_col_sum = f'sum__{name_col_batch}{str_suffix_summarized_metrics}'
            name_col_num_nonzero_values = f'num_nonzero_values__{name_col_batch}{str_suffix_summarized_metrics}'
            name_col_mean = f'mean__{name_col_batch}{str_suffix_summarized_metrics}'
            name_col_deviation = f'deviation__{name_col_batch}{str_suffix_summarized_metrics}'
            name_col_variance = f'variance__{name_col_batch}{str_suffix_summarized_metrics}'
            ...
            l_name_col_summarized = [ name_col_sum, name_col_num_nonzero_values, name_col_mean, name_col_deviation, name_col_variance ] # define name_col, fill_value, dtype of the columns that will be summarized.
            l_fill_value = [ 0, 0, 0, np.nan, np.nan ] 
            l_dtype = [ np.float64, np.float64, np.float64, np.float64, np.float64 ]
            flag_summarize_output_columns_already_exist = len( set( f"{name_layer}_{e}" for e in l_name_col_summarized ).difference( ax.meta.columns ) ) == 0 # retrieve a flag indicates the output columns of the summarize method are already exist in the metadata before initializing the columns
            for name_col, fill_value, dtype in zip( 
                l_name_col_summarized + [ name_col_ratio_for_selection, name_col_diff_for_selection, name_col_score_for_selection ],
                l_fill_value + [ 0, 0, 0 ], 
                ...
                ax.meta.initialize_column( name_col, dtype = dtype, shape_not_primary_axis = ( int_num_batches, ), chunks = ( 1, ), fill_value = fill_value ) # chunk size is 1 to allow calculation of highly variabele genes for each batch separately.
                dict_metadata = ax.meta.get_column_metadata( name_col ) # retrieve metadata
                dict_metadata[ 'l_labels_1' ] = l_name_batch # add cluster label information
            ...
            def summarize_sum_and_dev_for_each_batch( self, int_entry_of_axis_for_querying, arr_int_entries_of_axis_not_for_querying, arr_value ) :
                ...
                for batch in list( dict_batch_to_arr_value ) : # for the available 'batch'
                    ...
                    int_total_num_entries_not_indexed = dict_batch_to_count[ batch ] # retrieve the total number of entries for a batch
                    dict_summary[ name_col_sum ][ batch ] = np.sum( arr_value_of_a_batch ) if int_num_records > 30 else sum( arr_value_of_a_batch ) # if an input array has more than 30 elements, use np.sum to calculate the sum
                    dict_summary[ name_col_num_nonzero_values ][ batch ] = int_num_records 
                    dict_summary[ name_col_mean ][ batch ] = dict_summary[ name_col_sum ][ batch ] / int_total_num_entries_not_indexed # calculate the mean
                    arr_dev = ( arr_value_of_a_batch - dict_summary[ name_col_mean ][ batch ] ) ** 2 # calculate the deviation
                    dict_summary[ name_col_deviation ][ batch ] = np.sum( arr_dev ) if int_num_records > 30 else sum( arr_dev )
                    dict_summary[ name_col_variance ][ batch ] = dict_summary[ name_col_deviation ][ batch ] / ( int_total_num_entries_not_indexed - 1 ) if int_total_num_entries_not_indexed > 1 else np.nan
                return dict_summary
            ...
            name_col_sum_with_prefix, name_col_num_nonzero_values_with_prefix, name_col_mean_with_prefix, name_col_deviation_with_prefix, name_col_variance_with_prefix = list( f"{name_layer}_{e}" for e in [ name_col_sum, name_col_num_nonzero_values, name_col_mean, name_col_deviation, name_col_variance ] ) # add prefix
            name_col_ratio_for_selection_with_prefix, name_col_diff_for_selection_with_prefix, name_col_score_for_selection_with_prefix = list( f"{name_layer}_{e}" for e in [ name_col_ratio_for_selection, name_col_diff_for_selection, name_col_score_for_selection ] ) # add prefix
            
            # initialize
            def __map__calculate_highly_variable_features( p_r, p_s ) :
                zdf_meta_fork_safe = zdf_meta.get_fork_safe_version( flag_use_locking = False ) # get the fork-safe version, with loacking disabled (lock will be acquired in the main process, and since there will be no overlaps of chunks modified by individual worker processes, there is no need to acquire locks)
                while True :
                    """
                    ...
                    arr_mean = zdf_meta_fork_safe[ name_col_mean_with_prefix, None, batch ]
                    arr_var = zdf_meta_fork_safe[ name_col_variance_with_prefix, None, batch ]

                    # learn mean-variance relationship for the data
                    mask = ~ np.isnan( arr_var ) # exclude values containing np.nan
                    if mask.sum( ) == 0 : # contiue no valid data is available for fitting
                    ...
                    zdf_meta_fork_safe[ name_col_ratio_for_selection_with_prefix, None, batch ] = arr_ratio_of_variance_to_expected_variance_from_mean
                    zdf_meta_fork_safe[ name_col_diff_for_selection_with_prefix, None, batch ] = arr_diff_of_variance_to_expected_variance_from_mean
                    ...
                    def _calculate_weight( arr, thres : float, inplace : bool = True ) :
                        ...
                        if thres <= 0 :
                            return np.ones( len( arr ) ) # use the same weight for all the entries
                        if not inplace :
                            arr = deepcopy( arr )
                        arr[ arr > thres ] = thres 
                        ...
                        return arr
                    # calculate scores
                    arr_score = _calculate_weight( arr_mean, float_min_mean ) * _calculate_weight( arr_var, float_min_variance ) * arr_ratio_of_variance_to_expected_variance_from_mean * arr_diff_of_variance_to_expected_variance_from_mean # calculate the product of the ratio and difference of variance to expected variance for scoring and sorting highly variable features
                    zdf_meta_fork_safe[ name_col_score_for_selection_with_prefix, None, batch ] = arr_score # save the calculated scores

                    p_s.send( arr_score ) # return the result
                    del arr_ratio_of_variance_to_expected_variance_from_mean, arr_diff_of_variance_to_expected_variance_from_mean, arr_mean, arr_var
                zdf_meta_fork_safe.terminate_spawned_processes( ) # terminate the servers
            arr_score_accumulated = np.zeros( len( zdf_meta ), dtype = np.float64 ) # intialize the accumulated scores
            def __reduce__calculate_highly_variable_features( res ) :
                if isinstance( res, np.ndarray ) : # check whether the output is valid numpy array
                    res[ np.isnan( res ) ] = 0 # replace np.nan values with 0 values
                    arr_score_accumulated[ : ] += res

            # lock the output columns
            ax.meta.lock( name_col_ratio_for_selection_with_prefix, name_col_diff_for_selection_with_prefix, name_col_score_for_selection_with_prefix )
            ...
            ax.meta.unlock( name_col_ratio_for_selection_with_prefix, name_col_diff_for_selection_with_prefix, name_col_score_for_selection_with_prefix )
            
            # save combined scores as a column
            int_num_batch_active = len( l_batch_active ) # retrieve the number of active batches
            ax.meta[ f'{name_layer}__float_average_score_highly_variable_feature__{name_col_batch}{str_suffix_summarized_metrics}' ] = arr_score_accumulated / int_num_batch_active # calculate the average scores of each feature and save the average scores as a column

            # save the filter
            if int_num_highly_variable_features is None : # use all features with score > 0 
                arr_filter = arr_score_accumulated > 0 # use all features with score 
            else : # if valid number of entries were given
                arr_filter = np.zeros( ax.int_num_entries, dtype = bool ) # initialize the array
                arr_filter[ np.argsort( arr_score_accumulated )[ - int_num_highly_variable_features : ] ] = True # select the entries
                
            if name_col_filter is not None : # if the name of the output column is valid
                ax.save_as_filter( arr_filter, name_col_filter ) # save the filter using the given name
            ...
            if len( ax.meta ) < int_num_highly_variable_features :
                if self.verbose :
                    logger.info( f"[RamData.identify_highly_variable_features] there are only {len( ax.meta )} number of features satisfying the thresholds, 'int_num_highly_variable_features' will be modified." )
    ...
    def prepare_dimension_reduction_from_raw( 
        ...
        if flag_use_fast_mode :
            ...
            self.bc.meta.load_as_dict( name_col_total_count )
            dict_count = self.bc.meta.dict.pop( name_col_total_count ) # retrieve total counts for each barcode as a dictionary
            ...
            if name_layer_scaled is not None and name_col_variance is not None :
                ...
                # retrieve variance
                self.ft.meta.load_as_dict( name_col_variance )
                dict_var = self.ft.meta.dict.pop( name_col_variance ) # retrieve total counts for each barcode as a dictionary
    ...
    def get_expr( 
        ...
        if flag_using_cluster_label_for_retrieving_mask :
            ...
            dict_cat_to_l_index, dict_cat_count = ax_not_for_querying.meta.map_category_to_entries( name_col = name_col, flag_use_integer_representation_of_category = True, flag_return_dict_cat_to_num_entries = True )
    ...
    def train_pca( 
        ...
        if model is None :
            if self.verbose :
                logger.error( f"iPCA model '{name_model}' does not exist in the RamData models database, initializing the model." )
            ...
            if self.verbose :
                logger.info( f"existing iPCA model '{name_model}' found. the model will be updated using the currently defined inputs." )
            # parse the model
            ipca = model[ 'ipca' ] 

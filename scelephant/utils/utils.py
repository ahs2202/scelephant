...
def SemiSimulate_scData( path_folder_mtx_10x_input, path_folder_mtx_10x_output, float_prop_blur = 0.5, float_ratio_of_standard_deviation_to_expression_count = 0.3, int_num_copies = 30, int_num_threads = 30 ) :
    """ # 2022-07-16 00:44:53 
    ...
    path_file_mtx, path_file_feature, path_file_barcode = f"{path_folder_mtx_10x_input}matrix.mtx.gz", f"{path_folder_mtx_10x_input}features.tsv.gz", f"{path_folder_mtx_10x_input}barcodes.tsv.gz"

    # retrieve matrix metadata
    int_num_features, int_num_barcodes, int_num_records = bk.SC.MTX_10X_Retrieve_number_of_rows_columns_and_entries( path_folder_mtx_10x_input )
    ...
    def process_batch( index_batch, pipe_sender ) :
        ...
        bk.SC.MTX_10X_Barcode_add_prefix_or_suffix( path_file_barcode, f'{path_folder_temp}barcodes.{index_batch}.tsv.gz', barcode_suffix = f'|sim_{index_batch}' ) # add a batch-specific suffix
        ...
        newfile = gzip.open( f'{path_folder_temp}matrix.{index_batch}.mtx.gz', 'wb' ) # open an output matrix file
        with gzip.open( path_file_mtx, 'rb' ) as file :
            ...
            while True :
                ...
                newfile.write( ( ' '.join( tuple( map( str, [ index_row, index_col + int_total_num_cells_of_previous_batch, val ] ) ) ) + '\n' ).encode( ) ) # renumber barcode index          
                line = file.readline( ).decode( ) # read the next line
        newfile.close( )
    ...
    bk.Multiprocessing_Batch( np.arange( int_num_copies ).__iter__( ), process_batch, int_num_threads = int_num_threads ) # use 'int_num_copies' number of workers
    ...
    def combine_file( path_folder, name_file, name_ext, flag_run_in_bash_shell = True ) :
        ...
        if flag_run_in_bash_shell :
            os.system( f"cat {' '.join( list( df.path.values ) )} > {path_folder_temp}{name_file}.{name_ext}" ) # combine file using bash shell
        else :
            OS_Run( [ 'cat' ] + list( df.path.values ), path_file_stdout = f'{path_folder_temp}{name_file}.{name_ext}', stdout_binary = True ) # somehow using python pipe to save concatenated file takes a lot of memory.. therefore, 'flag_run_in_bash_shell' should be used
